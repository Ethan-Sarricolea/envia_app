...
class Capturer:
    ...
    def screnshot_desk(self,x1,y1,x2,y2):
        # Toma y recorta la captura
        screenshot = pyautogui.screenshot()
        screenshot = screenshot.crop((x1,y1,x2,y2))
        # Guarda 
        screenshot.save(f'src\screenshots\screenshot.jpg')
        ...
        for row in range(self.yin,200,self.rowSize):
            for i in range(0,5,1):
                if x==1290:
                    ...
                    self.screnshot_desk(x1=x,y1=row,x2=x+self.columnCoord[i],y2=y+self.rowSize)
                    x += self.columnCoord[i]
